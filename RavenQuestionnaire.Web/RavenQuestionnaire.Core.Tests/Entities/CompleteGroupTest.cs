...
namespace RavenQuestionnaire.Core.Tests.Entities
{
    ...
    public class CompleteGroupTest
    {
        ...
        public void SubscribeOnGroupAdd_CorrectData_GroupIsAdded()
        {
            ...
            CompleteGroup targetGroup = new CompleteGroup("some group")
                                            {Propagated = true};
        ...
        [Test]
        public void SubscribeOnGroupRemove_CorrectData_GroupIsRemoved()
        {
            /*CompleteQuestionnaireDocument document = new CompleteQuestionnaireDocument();
            CompleteQuestionnaire questionnaire = new CompleteQuestionnaire(new CompleteQuestionnaireDocument(), null);*/

            Mock<IComposite> document = new Mock<IComposite>();
            PropagatableCompleteGroup target = new PropagatableCompleteGroup() {PropogationPublicKey = Guid.NewGuid()};
            CompositeHandler handler = new CompositeHandler(document.Object);

            CompleteGroup targetGroup = new CompleteGroup("some group") { Propagated = true };
            document.Setup(x => x.Find<CompleteGroup>(targetGroup.PublicKey)).Returns(targetGroup);
            GroupObservable observeble = new GroupObservable(targetGroup.PublicKey, target.PublicKey);
            observeble.Subscribe(handler);
            handler.Remove(target);

//            document.Verify(x => x.Remove(targetGroup), Times.Once());
            document.Verify(x => x.Remove(It.Is<PropagatableCompleteGroup>(g=>g.PropogationPublicKey.Equals(target.PropogationPublicKey))), Times.Once());
        }
        [Test]
