        ...
        (function () {
            ...
            var Answer = function (key, surveyKey, parentKey, type, question, answer, answerOptions, isReadonly, isValid, isEnabled, isAnswered, isFlaged, comments) {
                ...
                self.addComment = function () {
                    ...
                    if (current) {
                        ...
                        datacontext.sendCommand(config.commands.setCommentCommand, {
                            user: input.user,
                            Comments: current,
                            completeQuestionnaireId: self.surveyKey,
                            propogationPublicKey: key.propagatekey,
                            questionPublickey: key.publicKey
                        }, {
                ...
                var answerQuestion = function () {
                    ...
                    datacontext.sendCommand(config.commands.setCommentCommand, {
                        ...
                        propogationPublicKey: key.propagatekey,
                        questionPublickey: key.publicKey,
                        completeAnswers: self.answerKeys,
                        completeAnswerValue: self.answer()
                        }, {
                            success: function(response) {
                                isSaving(false);
                            },
            ...
            var SurveyModel = function (questionnaire) {
                ...
                self.flagAnswer = function (answer, event) {
                    ...
                    var commandName = config.commands.setFlagCommand;

                    datacontext.sendCommand(commandName, {
                        completeQuestionnaireId: answer.surveyKey,
                        propogationPublicKey: answer.key.propagatekey,
                        questionPublickey: answer.key.publicKey,
                    ...
                    /*
                     var arg = {
                        questionId: answer.key.publicKey,
                        interviewId: answer.surveyKey,
                        userId: ""
                    };

                    var commandName = config.commands.removeFlagFromAnswer;
                    if (answer.isFlaged()) {
                        commandName = config.commands.setFlagToAnswer;
                    }
                    datacontext.sendCommand(commandName, arg, {
                        success: function (response) {
                            isSaving(false);
                        },
                        error: function (response) {
            ...
            $(document).ready(function () {
                ...
                viewModel = new SurveyModel(input.questionnaire || {});
