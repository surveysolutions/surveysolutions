...
{
    internal class when_export_all_data_from_sqlite_data_base : SqlToTabDataExportServiceTestContext
    {
        Establish context = () =>
        {
            ...
            RunCommand(sqlServiceFactory,
               string.Format("insert into [{0}] values ('{1}', 'Some action', 'nastya', 'inter', 'some data', 'some time');", "interview_actions", interviewId));
        };

        Because of = () =>
            sqlToTabDataExportService.GetDataFilesForQuestionnaire(questionnaireId, questionnaireVersion,"");
