...
namespace WB.Tests.Integration.CommandServiceTests
{
    ...
    {
        private class InvalidCommand : ICommand { public Guid CommandIdentifier => Guid.Empty; }

        private class Aggregate : IPlainAggregateRoot
        {
            public void SetId(Guid id) { }
            public void Handle(InvalidCommand command) { }
        }

        private class Validator : ICommandValidator<Aggregate, InvalidCommand>
        {
            public void Validate(Aggregate aggregate, InvalidCommand command)
            {
        ...
        Establish context = () =>
        {
            CommandRegistry
                .Setup<Aggregate>()
                .InitializesWith<InvalidCommand>(_ => Guid.Empty, aggregate => aggregate.Handle, config => config.ValidatedBy<Validator>());
        ...
        Because of = () =>
            exception = Catch.Exception(() => commandService.Execute(new InvalidCommand(), null));
