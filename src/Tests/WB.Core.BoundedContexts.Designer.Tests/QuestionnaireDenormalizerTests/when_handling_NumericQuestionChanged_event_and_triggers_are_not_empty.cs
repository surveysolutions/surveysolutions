...
{
    internal class when_handling_NumericQuestionChanged_event_and_triggers_are_not_empty : QuestionnaireDenormalizerTestsContext
    {
        Establish context = () =>
        {
            ...
            @event = CreateNumericQuestionChangedEvent(questionId, triggers: new List<Guid> { rosterGroupId });
            ...
            var questionFactory = Mock.Of<IQuestionFactory>(factory => factory.CreateQuestion(it.IsAny<QuestionData>()) == CreateNumericQuestion(questionId, "title"));
