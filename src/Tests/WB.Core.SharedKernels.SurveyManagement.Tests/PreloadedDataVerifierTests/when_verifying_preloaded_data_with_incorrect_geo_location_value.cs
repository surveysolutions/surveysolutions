...
{
    internal class when_verifying_preloaded_data_with_incorrect_geo_location_value : PreloadedDataVerifierTestContext
    {
        private Establish context = () =>
        {
            ...
            preloadedDataServiceMock.Setup(
                x => x.ParseQuestion(Moq.It.IsAny<string>(), Moq.It.IsAny<string>(), out outValue)).Returns(ValueParsingResult.AnswerAsGpsWasNotParsed);
        ...
        private Because of =
            () =>
                result = preloadedDataVerifier.Verify(questionnaireId, 1, new[] { preloadedDataByFile });

        private It should_result_has_1_error = () =>
            result.Count().ShouldEqual(1);

        private It should_return_single_PL0017_error = () =>
            result.First().Code.ShouldEqual("PL0017");
