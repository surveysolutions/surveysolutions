...
{
    internal class when_Completed_recived : InterviewExportedDataEventHandlerTestContext
    {
        Establish context = () =>
        {
        ...
        Because of = () =>
            interviewExportedDataDenormalizer.Handle(CreatePublishableEvent(() => new InterviewCompleted(Guid.NewGuid(), DateTime.Now),
                interviewId));

        It should_Completed_action_be_present_in_list_of_actions = () =>
            interviewActionLog.Actions[0].Action.ShouldEqual(InterviewExportedAction.Completed);
