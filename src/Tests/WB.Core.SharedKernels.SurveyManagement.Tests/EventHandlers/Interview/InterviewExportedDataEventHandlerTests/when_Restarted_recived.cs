...
{
    internal class when_Restarted_recived : InterviewExportedDataEventHandlerTestContext
    {
        Establish context = () =>
        {
        ...
        Because of = () =>
            interviewExportedDataDenormalizer.Handle(CreatePublishableEvent(() => new InterviewRestarted(Guid.NewGuid(), DateTime.Now),
                interviewId));

        It should_Restarted_action_be_present_in_list_of_actions = () =>
            interviewActionLog.Actions[0].Action.ShouldEqual(InterviewExportedAction.Restarted);
