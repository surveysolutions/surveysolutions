...
{
    internal class when_handling_InterviewRejected_event : InterviewSummaryEventHandlerFunctionalTestsContext
    {
        ...
        Because of = () => 
            viewModel = denormalizer.Update(viewModel, Create.InterviewRejectedEvent(userId: responsibleId, comment: comment));
        ...
        It should_commented_statuses_history_first_item_status_be_equal_to_interview_status_rejected_by_supervisor = () =>
            viewModel.CommentedStatusesHistory.Single().Status.ShouldEqual(InterviewStatus.RejectedBySupervisor);
