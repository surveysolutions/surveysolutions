...
{
    internal class when_handling_InterviewRejectedByHQ_event : InterviewSummaryEventHandlerFunctionalTestsContext
    {
        ...
        Because of = () => 
            viewModel = denormalizer.Update(viewModel, Create.InterviewRejectedByHQEvent(userId: responsibleId, comment: comment));
        ...
        It should_commented_statuses_history_first_item_status_be_equal_to_interview_status_rejected_by_headquarters = () =>
            viewModel.CommentedStatusesHistory.Single().Status.ShouldEqual(InterviewStatus.RejectedByHeadquarters);
