...
namespace WB.Core.BoundedContexts.Capi.Tests.Views.InterviewViewModelTests
{
    internal class when_disabling_answered_invalid_question_question_in_interview : InterviewViewModelTestContext
    {
        Establish context = () =>
        {
            ...
            interviewViewModel.SetQuestionValidity(new InterviewItemId(targetQuestionId, new decimal[0]), false);
        };

        Because of = () =>
             interviewViewModel.SetQuestionStatus(new InterviewItemId(targetQuestionId, new decimal[0]), false);
