...
namespace WB.Tests.Unit.Designer.BoundedContexts.Designer.CodeGeneratorTests
{
    internal class when_creating_models_with_single_question : CodeGeneratorTestsContext
    {
        ...
        [NUnit.Framework.Test] public void should_create_single_question_model () 
        {
            QuestionTemplateModel question = model.AllQuestions.Single(x => x.Id == Id.gA);
            question.VariableName.Should().Be("single");
            question.ValidationExpressions.FirstOrDefault().ValidationExpression.Should().Be("single validation");
            question.Condition.Should().Be("single condition");
