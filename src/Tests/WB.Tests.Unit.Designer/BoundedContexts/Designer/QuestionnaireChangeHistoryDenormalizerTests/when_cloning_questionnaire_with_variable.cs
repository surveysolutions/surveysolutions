...
namespace WB.Tests.Unit.Designer.BoundedContexts.Designer.QuestionnaireChangeHistoryDenormalizerTests
{
    internal class when_cloning_questionnaire_with_variable : QuestionnaireChangeHistoryDenormalizerTestContext
    {
        Establish context = () =>
        {
            ...
            denormalizer.Handle(Create.Event.ToPublishedEvent(Create.Event.QuestionnaireCloned(questionnaire)));
        };

        Because of = () =>
        {
            var variableDeleted = Create.Event.ToPublishedEvent(Create.Event.VariableDeleted(variableId));
