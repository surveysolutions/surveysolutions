...
{
    internal class when_updating_attachment : QuestionnaireChangeHistoryDenormalizerTestContext
    {
        Establish context = () =>
        {
        ...
        Because of = () => 
            questionnaireChangeHistoryDenormalizer.Handle(Create.Event.AttachmentUpdated(questionnaireId: questionnaireId, entityId: attachmentId));

        It should_create_single_history_record = () =>
        {
            var historyRecord = questionnaireChangeRecordStorage.Dictionary.Values.Single();
            historyRecord.ActionType.ShouldEqual(QuestionnaireActionType.Update);
