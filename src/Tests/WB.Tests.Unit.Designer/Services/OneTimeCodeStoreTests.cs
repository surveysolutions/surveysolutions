...
namespace WB.Tests.Unit.Designer.Services
{
    ...
    public class OneTimeCodeStoreTests
    {
        ...
        {
            var store = new InMemoryOneTimeCodeStore();
            await store.SaveAsync(MakeEntity("code2"));

            var usedAt = DateTime.UtcNow;
            await store.TryMarkAsUsedAsync("code2", usedAt);

            var entity = await store.GetAsync("code2");
            Assert.That(entity!.Used, Is.True);
            Assert.That(entity.UsedAt, Is.EqualTo(usedAt).Within(TimeSpan.FromMilliseconds(1)));
        }
