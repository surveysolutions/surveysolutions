...
namespace WB.Tests.Unit.Designer.ClassificationsTests
{
    ...
    public class ClassificationStorageTests
    {
        ...
        public async Task When_getting_categories_for_user()
        {
            ...
            var orders = Enumerable.Range(1, Constants.MaxRosterRowCount + 10).OrderBy(x => Guid.NewGuid()).ToArray();
            entities.AddRange(
                Enumerable
                    .Range(1, Constants.MaxRosterRowCount + 10)
            ...
            Assert.That(categories.Count, Is.EqualTo(Constants.MaxRosterRowCount));
            CollectionAssert.IsOrdered(categories.Select(x => x.Order));
            Assert.That(categories.Max(x => x.Order), Is.EqualTo(Constants.MaxRosterRowCount));
