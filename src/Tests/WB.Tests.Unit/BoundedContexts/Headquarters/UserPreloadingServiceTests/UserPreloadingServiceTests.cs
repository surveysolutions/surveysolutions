...
namespace WB.Tests.Unit.BoundedContexts.Headquarters.UserPreloadingServiceTests
{
    ...
    internal class UserPreloadingServiceTests
    {
            ...
            FinishValidationProcess_When_process_is_in_validating_state_Then_validation_process_should_be_finished()
        {
            var processId = "aaa";
            var userPreloadingProcessStorage = new TestPlainStorage<UserPreloadingProcess>();
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId, state: UserPrelodingState.Validating), processId);
            var userPreloadingService =
            ...
            FinishPreloadingProcess_When_process_is_in_creating_users_state_Then_process_should_be_finished()
        {
            var processId = "aaa";
            var userPreloadingProcessStorage = new TestPlainStorage<UserPreloadingProcess>();
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId, state: UserPrelodingState.CreatingUsers), processId);
            var userPreloadingService =
                this.CreateUserPreloadingService(userPreloadingProcessStorage: userPreloadingProcessStorage);
            ...
            EnqueueForValidation_When_process_is_in_Uploaded_state_Then_process_should_be_ready_for_validation()
        {
            var processId = "aaa";
            var userPreloadingProcessStorage = new TestPlainStorage<UserPreloadingProcess>();
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId, state: UserPrelodingState.Uploaded), processId);
            var userPreloadingService =
                this.CreateUserPreloadingService(userPreloadingProcessStorage: userPreloadingProcessStorage);
            ...
            EnqueueForUserCreation_When_process_is_in_Validated_state_Then_process_should_be_ready_for_user_creation()
        {
            var processId = "aaa";
            var userPreloadingProcessStorage = new TestPlainStorage<UserPreloadingProcess>();
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId, state: UserPrelodingState.Validated), processId);
            var userPreloadingService =
                this.CreateUserPreloadingService(userPreloadingProcessStorage: userPreloadingProcessStorage);
            ...
            DeQueuePreloadingProcessIdReadyToBeValidated_When_2_processes_in_ready_to_be_validated_state_are_present_Then_oldest_process_id_should_be_returned()
        {
            ...
            var processId2 = "bbb";
            var userPreloadingProcessStorage = new TestPlainStorage<UserPreloadingProcess>();
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId1, state: UserPrelodingState.ReadyForValidation), processId1);
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId2, state: UserPrelodingState.ReadyForValidation), processId2);
            var userPreloadingService =
                this.CreateUserPreloadingService(userPreloadingProcessStorage: userPreloadingProcessStorage);
            ...
            DeQueuePreloadingProcessIdReadyToCreateUsers_When_2_processes_in_ReadyForUserCreation_state_are_present_Then_oldest_process_id_should_be_returned()
        {
            ...
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId1, state: UserPrelodingState.ReadyForUserCreation), processId1);
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId2, state: UserPrelodingState.ReadyForUserCreation), processId2);
            var userPreloadingService =
                this.CreateUserPreloadingService(userPreloadingProcessStorage: userPreloadingProcessStorage);
            ...
            PushVerificationError_When_process_is_in_validating_state_Then_one_error_should_be_stored()
        {
            var processId = "aaa";
            var userPreloadingProcessStorage = new TestPlainStorage<UserPreloadingProcess>();
            userPreloadingProcessStorage.Store(Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId,
            ...
            PushVerificationError_When_process_is_in_validating_state_with_100_stored_errors_Then_one_UserPreloadingException_should_be_throwns()
        {
            var processId = "aaa";
            var userPreloadingProcessStorage = new TestPlainStorage<UserPreloadingProcess>();
            var userPreloadingProcess = Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId,
                state: UserPrelodingState.Validating);

            for (int i = 0; i < 100; i++)
            {
                userPreloadingProcess.VerificationErrors.Add(Create.Other.UserPreloadingVerificationError());
            }
            ...
            IncrementCreatedUsersCount_When_process_is_in_CreatingUsers_state_Then_created_user_count_should_be_updated()
        {
            var processId = "aaa";
            var userPreloadingProcessStorage = new TestPlainStorage<UserPreloadingProcess>();
            var userPreloadingProcess = Create.Other.UserPreloadingProcess(userPreloadingProcessId: processId, recordsCount: 1,
            ...
            GetUserRoleFromDataRecord_Then_supervisor_interviewer_or_undefined_role_should_be_returned()
        {
            ...
            var supervisor =
                userPreloadingService.GetUserRoleFromDataRecord(Create.Other.UserPreloadingDataRecord(role: "Supervisor"));

            var interviewer =
                userPreloadingService.GetUserRoleFromDataRecord(Create.Other.UserPreloadingDataRecord(role: "interviewer"));

            var undefined =
               userPreloadingService.GetUserRoleFromDataRecord(Create.Other.UserPreloadingDataRecord(role: "aaas"));
            ...
            IRecordsAccessorFactory recordsAccessorFactory = null)
        {
            return
                new UserPreloadingService(
                    ...
                    Create.Other.UserPreloadingSettings());
        }
    }
}