...
namespace WB.Tests.Unit.BoundedContexts.Headquarters.EventHandler
{
    ...
    public class AbstractDenormalizerTests
    {
        ...
        public void should_not_save_state_for_events_that_are_not_handled_by_denormalizer()
        {
            ...
            var storageMock = new Mock<IReadSideRepositoryWriter<InterviewEntity>>();
            ...
            storageMock.Verify(x => x.Store(It.IsAny<InterviewEntity>(), It.IsAny<string>()), Times.Never, "Event is not handled by denormalizer and results should not be stored");
        }
    }
}