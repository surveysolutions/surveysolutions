...
{
    internal class when_restoring_interview_state_from_sync_package_with_linked_multi_question : StatefulInterviewTestsContext
    {
        Establish context = () =>
        {
        ...
        It should_rise_InterviewAnswersFromSyncPackageRestored_event_with_right_answer_type_and_other_fields = () =>
        {
            ...
            answerDto.Type.ShouldEqual(AnswerType.LinkedMultiOptionAnswer);
            answerDto.Id.ShouldEqual(questionId);
            answerDto.RosterVector.ShouldEqual(rosterVector);
            answerDto.Answer.ShouldEqual(answer);
        };

        static readonly object answer = new decimal[][] { new[] { 1m, 0 }, new[] { 1m, 4m } };
