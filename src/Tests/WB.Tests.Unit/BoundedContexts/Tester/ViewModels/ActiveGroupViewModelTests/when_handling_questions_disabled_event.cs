...
{
    internal class when_handling_questions_disabled_event : ActiveGroupViewModelTestContext
    {
        Establish context = () =>
        {
        ...
        Because of = () =>
            activeGroup.Handle(Create.Event.QuestionsDisabled(disabledQuestionIdentity.Id, disabledQuestionIdentity.RosterVector));

        It should_publish_message_about_item_update_in_position_1 = () =>
            messengerMock.Verify(
                x => x.Publish(Moq.It.Is<UpdateInterviewEntityStateMessage>(param => param.ElementPosition == 4)),
                Times.Once);
        ...
        protected static Identity disabledQuestionIdentity = Create.Identity(Guid.Parse("66666666666666666666666666666666"), new decimal[] { 1, 2 });
