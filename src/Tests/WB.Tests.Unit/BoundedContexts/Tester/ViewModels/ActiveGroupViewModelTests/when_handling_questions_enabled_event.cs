...
{
    internal class when_handling_questions_enabled_event : ActiveGroupViewModelTestContext
    {
        Establish context = () =>
        {
        ...
        Because of = () =>
            activeGroup.Handle(Create.Event.QuestionsEnabled(enabledQuestionIdentity.Id, enabledQuestionIdentity.RosterVector));

        It should_publish_message_about_item_update_in_position_1 = () =>
            messengerMock.Verify(
                x => x.Publish(Moq.It.Is<UpdateInterviewEntityStateMessage>(param => param.ElementPosition == 3)),
                Times.Once);
        ...
        protected static Identity enabledQuestionIdentity = Create.Identity(Guid.Parse("55555555555555555555555555555555"), new decimal[] { 1, 2 });
