...
namespace WB.Tests.Unit.BoundedContexts.Capi.InterviewViewModelTests
{
    internal class when_setting_answer_to_nested_group_inside_roster : InterviewViewModelTestContext
    {
        ...
        Because of = () =>
            interviewViewModel.SetAnswer(ConversionHelper.ConvertIdAndRosterVectorToString(questionInNestedGroupId, new decimal[] { 0 }), 3);
        ...
        It should_count_of_answered_questions_in_nested_group_screen_be_equal_to_1 = () =>
            ((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[ConversionHelper.ConvertIdAndRosterVectorToString(nestedGroupId, new decimal[] { 0 })]).Answered.ShouldEqual(1);

        It should_count_of_total_questions_in_nested_group_screen_be_equal_to_1 = () =>
            ((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[ConversionHelper.ConvertIdAndRosterVectorToString(nestedGroupId, new decimal[] { 0 })]).Total.ShouldEqual(1);

        It should_answer_on_question_in_nested_group_be_equal_3_ = () =>
            ((ValueQuestionViewModel)((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[ConversionHelper.ConvertIdAndRosterVectorToString(nestedGroupId, new decimal[] { 0 })]).Items[0]).AnswerObject.ShouldEqual(3);
