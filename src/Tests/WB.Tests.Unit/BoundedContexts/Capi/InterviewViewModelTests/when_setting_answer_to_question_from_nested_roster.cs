...
namespace WB.Tests.Unit.BoundedContexts.Capi.InterviewViewModelTests
{
    internal class when_setting_answer_to_question_from_nested_roster : InterviewViewModelTestContext
    {
        ...
        Because of = () =>
            interviewViewModel.SetAnswer(InterviewItemId.ConvertIdAndRosterVectorToString(questionInNestedGroupId, new decimal[] { 0, 0 }), 3);
        ...
        It should_count_of_answered_questions_in_nested_group_screen_be_equal_to_1 = () =>
            ((QuestionnaireGridViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(nestedRosterId, new decimal[] { 0 })]).Answered.ShouldEqual(1);

        It should_count_of_answered_questions_in_nested_roster_row_screen_be_equal_to_1 = () =>
            ((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(nestedRosterId, new decimal[] { 0, 0 })]).Answered.ShouldEqual(1);

        It should_count_of_total_questions_in_nested_group_screen_be_equal_to_1 = () =>
            ((QuestionnaireGridViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(nestedRosterId, new decimal[] { 0 })]).Total.ShouldEqual(1);

        It should_answer_on_question_in_nested_group_be_equal_3_ = () =>
            ((ValueQuestionViewModel)((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(nestedRosterId, new decimal[] { 0, 0 })]).Items[0]).AnswerObject.ShouldEqual(3);
