...
namespace WB.Tests.Unit.BoundedContexts.Capi.InterviewViewModelTests
{
    internal class when_all_groups_disabled_and_1_group_enabling_with_enabled_question_and_disabled_child_groups : InterviewViewModelTestContext
    {
        ...
        Because of = () =>
            interviewViewModel.SetScreenStatus(InterviewItemId.ConvertIdAndRosterVectorToString(nestedGroupId, new decimal[0]), true);

        It should_chapter_be_enabled = () =>
            ((QuestionnaireScreenViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(chapterId, new decimal[0])]).Enabled.ShouldEqual(true);

        It should_nested_group_be_enabled = () =>
            ((QuestionnaireScreenViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(nestedGroupId, new decimal[0])]).Enabled.ShouldEqual(true);
        ...
        It should_quesition_inside_nested_group_be_enabled = () =>
            ((QuestionViewModel)
                ((QuestionnaireScreenViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(nestedGroupId, new decimal[0])]).Items[0])
        ...
        It should_nested_inner_group_be_disabled = () =>
            ((QuestionnaireScreenViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(nestedGroupInnterGroupId, new decimal[0])]).Enabled.ShouldEqual(false);

        It should_nested_inner_group_be_disabled_as_item_inside_chapter = () =>
            ((QuestionnaireNavigationPanelItem)((QuestionnaireScreenViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(nestedGroupId, new decimal[0])]).Items[1]).Enabled.ShouldEqual(false);

        It should_quesition_inside_chapter_be_enabled = () =>
           ((QuestionViewModel)
               ((QuestionnaireScreenViewModel)interviewViewModel.Screens[InterviewItemId.ConvertIdAndRosterVectorToString(chapterId, new decimal[0])]).Items[1])
               .Status.HasFlag(QuestionStatus.ParentEnabled).ShouldEqual(true);
