...
namespace WB.Tests.Unit.BoundedContexts.Capi.InterviewViewModelTests
{
    internal class when_add_roster_row_method_called : InterviewViewModelTestContext
    {
        ...
        It should_contain_screen_with_added_roster_row = () =>
            interviewViewModel.Screens.Keys.ShouldContain(InterviewViewModel.ConvertIdAndRosterVectorToString(rosterId1, new decimal[] { 0 }));

        It should_contain_roster_with_first_row_with_empty_Screen_name = () =>
           ((QuestionnaireGridViewModel)interviewViewModel.Screens[InterviewViewModel.ConvertIdAndRosterVectorToString(rosterId1, new decimal[0])]).Rows.First().ScreenName.ShouldEqual(string.Empty);

        It should_contain_screen_with_3_breadcrumbs = () =>
            ((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[InterviewViewModel.ConvertIdAndRosterVectorToString(rosterId1, new decimal[] { 0 })]).Breadcrumbs.Count().ShouldEqual(3);

        It should_contain_roster_screen_with_empty_screen_title = () =>
           ((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[InterviewViewModel.ConvertIdAndRosterVectorToString(rosterId1, new decimal[] { 0 })]).ScreenName.ShouldEqual(string.Empty);

        It should_contain_screen_with_last_breadcrumb_of_current_Screen = () =>
            ((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[InterviewViewModel.ConvertIdAndRosterVectorToString(rosterId1, new decimal[] { 0 })]).Breadcrumbs.Last().ShouldEqual(new InterviewItemId(rosterId1, new decimal[] { 0 }));

        It should_contain_screen_with_2nd_breadcrumb_of_grid_Screen = () =>
            ((QuestionnairePropagatedScreenViewModel)interviewViewModel.Screens[InterviewViewModel.ConvertIdAndRosterVectorToString(rosterId1, new decimal[] { 0 })])
