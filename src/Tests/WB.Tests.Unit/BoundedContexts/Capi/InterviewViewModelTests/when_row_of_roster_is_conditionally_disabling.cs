...
namespace WB.Tests.Unit.BoundedContexts.Capi.InterviewViewModelTests
{
    internal class when_row_of_roster_is_conditionally_disabling : InterviewViewModelTestContext
    {
        ...
        Because of = () => interviewViewModel.SetScreenStatus(new InterviewItemId(rosterGroupId, new decimal[]{0}), false);

        It should_roster_be_enabled = () =>
            ((QuestionnaireGridViewModel)interviewViewModel.Screens[new InterviewItemId(rosterGroupId, new decimal[0])]).Enabled.ShouldEqual(true);
        ...
        It should_roster_answered_question_count_be_equal_to_1 = () =>
           ((QuestionnaireGridViewModel)interviewViewModel.Screens[new InterviewItemId(rosterGroupId, new decimal[0])]).Answered.ShouldEqual(0);
