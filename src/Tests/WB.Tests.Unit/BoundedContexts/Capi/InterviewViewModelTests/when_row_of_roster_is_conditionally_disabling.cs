...
namespace WB.Tests.Unit.BoundedContexts.Capi.InterviewViewModelTests
{
    internal class when_row_of_roster_is_conditionally_disabling : InterviewViewModelTestContext
    {
        ...
        Because of = () => interviewViewModel.SetScreenStatus(InterviewViewModel.ConvertIdAndRosterVectorToString(rosterGroupId, new decimal[] { 0 }), false);

        It should_roster_be_enabled = () =>
            ((QuestionnaireGridViewModel)interviewViewModel.Screens[InterviewViewModel.ConvertIdAndRosterVectorToString(rosterGroupId, new decimal[0])]).Enabled.ShouldEqual(true);
