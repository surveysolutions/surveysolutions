...
{
    internal class when_verifying_questionnaire_that_has_group_with_condition_expression_with_more_than_10000_chars_length : QuestionnaireVerifierTestsContext
    {
        Establish context = () =>
        {
            questionnaire = CreateQuestionnaireDocument(new IComposite[]
            {
                Create.Group(groupId: parentGroupId, children: new IComposite[]
                {
        ...
        It should_return_error_with_code__WB0094 = () =>
            resultErrors.First().Code.ShouldEqual("WB0094");
        ...
        It should_return_error_with_one_references_with_id_equals_groupId = () =>
            resultErrors.First().References.First().Id.ShouldEqual(groupId);

        private It should_not_call_GenerateProcessorStateAssembly = () =>
            questionnireExpressionProcessorGeneratorMock.Verify(x => x.GenerateProcessorStateAssemblyForVersion(Moq.It.IsAny<QuestionnaireDocument>(), Moq.It.IsAny<ExpressionsEngineVersion>(), out generationResult), Times.Never);

        private static IEnumerable<QuestionnaireVerificationError> resultErrors;
        private static QuestionnaireVerifier verifier;
        private static QuestionnaireDocument questionnaire;
        private static Guid groupId = Guid.Parse("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD");
