...
{
    internal class when_verifying_preloaded_data_with_incorrect_integer_value : PreloadedDataVerifierTestContext
    {
        private Establish context = () =>
        {
            ...
            preloadedDataServiceMock.Setup(x => x.GetColumnIndexesGoupedByQuestionVariableName(preloadedDataByFile))
                .Returns(new Dictionary<string, Tuple<string, int>[]> { { "q1", new[] { new Tuple<string, int>("q1", 1) } } });

            KeyValuePair<Guid, object> outValue;

            preloadedDataServiceMock.Setup(
                x => x.ParseQuestion(Moq.It.IsAny<string>(), Moq.It.IsAny<string>(), Moq.It.IsAny<IQuestion>(), out outValue)).Returns(ValueParsingResult.AnswerAsIntWasNotParsed);
        ...
        private It should_return_single_PL0018_error = () =>
            result.Errors.First().Code.ShouldEqual("PL0018");
