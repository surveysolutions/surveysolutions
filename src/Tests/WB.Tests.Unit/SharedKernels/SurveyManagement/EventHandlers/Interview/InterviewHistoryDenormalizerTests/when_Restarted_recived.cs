...
{
    internal class when_Restarted_recived : InterviewHistoryDenormalizerTestContext
    {
        Establish context = () =>
        {
        ...
        Because of = () =>
            interviewHistoryView = interviewExportedDataDenormalizer.Update(interviewHistoryView, CreatePublishableEvent(() => new InterviewStatusChanged(InterviewStatus.Restarted,"rest"),
                interviewId));

        It should_action_of_first_record_be_Restarted = () =>
            interviewHistoryView.Records[0].Action.ShouldEqual(InterviewHistoricalAction.Restarted);
