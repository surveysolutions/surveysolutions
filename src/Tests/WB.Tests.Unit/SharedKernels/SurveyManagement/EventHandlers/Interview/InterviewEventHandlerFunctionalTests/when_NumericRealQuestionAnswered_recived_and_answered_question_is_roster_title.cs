...
namespace WB.Tests.Unit.SharedKernels.SurveyManagement.EventHandlers.Interview.InterviewEventHandlerFunctionalTests
{
    internal class when_NumericRealQuestionAnswered_recived_and_answered_question_is_roster_title : InterviewEventHandlerFunctionalTestContext
    {
        ...
        It should_roster_title_be_equal_to_recived_answer = () =>
            viewState.Levels["0"].RosterRowTitles[rosterGroupId].ShouldEqual(decimalAnswer.ToString());

        It should_answer_on_head_question_be_equal_to_recived_answer = () =>
            viewState.Levels["0"].GetAllQuestions().First(q => q.Id == rosterTitleQuestionId).Answer.ShouldEqual(decimalAnswer);
