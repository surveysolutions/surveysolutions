...
namespace WB.Tests.Unit.SharedKernels.SurveyManagement.SurveysAndStatusesReportTests
{
    internal class when_requesting_report_by_responsible : SurveysAndStatusesReportTestsContext
    {
        Establish context = () =>
        {
            ...
            {
                Create.InterviewSummary(questionnaireId: questionnaireId, status: InterviewStatus.Completed, responsibleId: userId, responsibleName: responsibleName),
                Create.InterviewSummary(questionnaireId: questionnaireId, status: InterviewStatus.Completed, responsibleId: userId, responsibleName: responsibleName),
        ...
        Because of = () => report = reportFactory.Load(new SurveysAndStatusesReportInputModel { ResponsibleName = responsibleName });
