...
namespace WB.Tests.Unit.SharedKernels.SurveyManagement.SupervisorTeamsAndStatusesReportTests
{
    internal class when_report_requested_by_viewer : SupervisorTeamsAndStatusesReportContext
    {
        Establish context = () =>
        {
            ...
            {
                Create.Other.InterviewSummary(responsibleId: responsible, teamLeadId: viewerId, status: InterviewStatus.Completed),
                Create.Other.InterviewSummary(responsibleId: responsible, teamLeadId: viewerId, status: InterviewStatus.Completed),
                Create.Other.InterviewSummary(responsibleId: responsible, teamLeadId: Guid.NewGuid(), status: InterviewStatus.Completed),
            };
