...
{
    class when_exporting_approved_interviews : InterviewActionsExporterTestsContext
    {
        Establish context = () =>
        {
            ...
            actionsExporter =
                CreateExporter(csvWriter: csvWriterMock.Object,
        ...
        Because of = () =>
            actionsExporter.ExportApproved(new QuestionnaireIdentity(questionnaireId, questionnaireVersion), "", new Progress<int>());

        It should_record_one_completedaction = () =>
            csvWriterMock.Verify(x => 
            x.WriteData(Moq.It.IsAny<string>(), Moq.It.Is<IEnumerable<string[]>>(s => s.Any(c => c.Contains("Completed"))), Moq.It.IsAny<string>()), Times.Once);

        It should_record_one_one_approved_action = () =>
            csvWriterMock.Verify(x => 
            x.WriteData(Moq.It.IsAny<string>(), Moq.It.Is<IEnumerable<string[]>>(s => s.Any(c => c.Contains("ApprovedByHeadquarter"))), Moq.It.IsAny<string>()), Times.Once);
        ...
        private static readonly Mock<ICsvWriter> csvWriterMock = new Mock<ICsvWriter>();
    }
}