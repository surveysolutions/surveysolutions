...
namespace WB.Tests.Unit.SharedKernels.SurveyManagement.SynchronizationDenormalizerTests
{
    internal class when_interview_status_changed_on_RejectedBySupervisor : InterviewSynchronizationDenormalizerTestsContext
    {
        Establish context = () =>
        {
            var storedInterview =  Create.Entity.InterviewData(
        ...
        Because of = () =>
            denormalizer.Handle(Create.PublishedEvent.InterviewStatusChanged(interviewId, InterviewStatus.RejectedBySupervisor, comments, eventId: Guid.Parse(partialPackageId)));
