...
namespace WB.Tests.Unit.SharedKernels.SurveyManagement.SynchronizationDenormalizerTests
{
    internal class when_interview_status_changed_on_Deleted : InterviewSynchronizationDenormalizerTestsContext
    {
        Establish context = () =>
        {
            var storedInterview = Create.Entity.InterviewSummary();
        ...
        Because of = () =>
            denormalizer.Handle(Create.PublishedEvent.InterviewStatusChanged(interviewId, InterviewStatus.Deleted, comments, eventId: Guid.Parse(partialPackageId)));
        ...
        It should_create_interview_package = () =>
            interviewPackageStorageWriterMock.Verify(x => x.Store(
                ...
                Moq.It.Is<string>(id => id.StartsWith(partialPackageId))), 
                Times.Once);
