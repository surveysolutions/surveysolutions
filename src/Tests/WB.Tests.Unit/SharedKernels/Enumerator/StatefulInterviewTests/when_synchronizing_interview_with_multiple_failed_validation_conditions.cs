...
namespace WB.Tests.Unit.SharedKernels.Enumerator.StatefulInterviewTests
{
    internal class when_synchronizing_interview_with_multiple_failed_validation_conditions : StatefulInterviewTestsContext
    {
        private Establish context = () =>
        {
            ...
            var questionnaire = Create.Entity.QuestionnaireDocumentWithOneChapter(id: questionnaireId, children: 
                Create.Entity.FixedRoster(fixedRosterIdentity.Id, fixedTitles: new[] { new FixedRosterTitle(1, "fixed") }, children: new IComposite[]
                {
                    Create.Entity.FixedRoster(fixedNestedRosterIdentity.Id, fixedTitles: new[] {new FixedRosterTitle(0, "nested fixed")}, children: new IComposite[]
                    {
            ...
            Action<Mock<IInterviewLevel>> setupLevel = (levelMock) =>
            {
                levelMock
                    .Setup(x => x.GetValidationExpressions(questionIdentity))
                    .Returns(new Func<bool>[]
                    {
                        () => false,
                        () => true,
                        () => false
