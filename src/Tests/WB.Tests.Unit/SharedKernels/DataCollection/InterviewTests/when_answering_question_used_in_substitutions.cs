    ...
    internal class when_answering_question_used_in_substitutions : InterviewTestsContext
    {
        Establish context = () =>
        {
            ...
            QuestionnaireDocument questionnaire = CreateQuestionnaireDocumentWithOneChapter(new Group("top level fixed group")
            {
                ...
                {
                    Create.TextQuestion(questionId: substitutedQuestionId, variable: "subst"),

                    Create.TextQuestion(questionId: questionA, text: "question A %subst%"),
                    Create.TextQuestion(questionId: questionB, text: "question B %subst%"),
                    Create.StaticText(publicKey: staticTextId, text: "static text %subst%"),
                }
            });

            IPlainQuestionnaireRepository questionnaireRepository = CreateQuestionnaireRepositoryStubWithOneQuestionnaire(questionnaireId, new PlainQuestionnaire(questionnaire, 1));
