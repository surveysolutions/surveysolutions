...
{
    internal class when_answer_single_option_linked_question_received_by_interviewer : InterviewTestsContext
    {
        Establish context = () =>
        {
            ...
            var questionnaire = CreateQuestionnaireDocumentWithOneChapter(
                ...
                Create.SingleOptionQuestion(questionId: questionId, linkedToQuestionId: textQuestionId));

            var questionnaireRepository = CreateQuestionnaireRepositoryStubWithOneQuestionnaire(questionnaireId,
                new PlainQuestionnaire(questionnaire, 1));

            interview = CreateInterview(questionnaireId: questionnaireId, questionnaireRepository: questionnaireRepository);
            interview.AnswerTextQuestion(userId, textQuestionId, new decimal[0], DateTime.Now, "answer");
            interview.Apply(new InterviewReceivedByInterviewer());
        };

        Because of = () =>
            exception = Catch.Only<InterviewException>(() => interview.AnswerSingleOptionLinkedQuestion(userId, questionId, new decimal[0], DateTime.Now, new decimal[0]));
