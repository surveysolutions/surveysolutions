    ...
    internal class when_substuting_question_title_inside_roster_with_reference_to_roster_size_question : InterviewTestsContext
    {
        private Establish context = () =>
        {
            ...
            QuestionnaireDocument questionnaire = CreateQuestionnaireDocumentWithOneChapter(new Group("root")
            {
                ...
                {
                    Create.Other.NumericIntegerQuestion(id: rosterSizeQuestionId, variable: "subst"),
                    Create.Other.Roster(rosterSizeSourceType: RosterSizeSourceType.Question, 
                        rosterSizeQuestionId: rosterSizeQuestionId, 
                        title: "roster",
                        children: 
                            new List<IComposite>
                            {
                                Create.Other.TextQuestion(questionId: questionA, text: "this is %subst% A"),
                                Create.Other.TextQuestion(questionId: questionB, text: "this is %subst% B")
                            })
                }
            });
