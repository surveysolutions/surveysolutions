...
{
    internal class when_answer_on_qr_barcode_question_and_interview_approved_by_hq : InterviewAnswersCommandValidatorTestsContext
    {
        Establish context = () =>
        {
        ...
        Because of = () => exception = Catch.Only<InterviewException>(
            () => commandValidator.Validate(interview,
                    Create.AnswerQRBarcodeQuestionCommand(interviewId: interviewId, userId: responsibleId)));
