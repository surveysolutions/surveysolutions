...
namespace WB.Core.SharedKernels.QuestionnaireVerification.Tests.QuestionnaireVerifierTests
{
    internal class when_verifying_questionnaire_with_roster_that_have_roster_level_more_than_4_and_rosters_have_different_roster_size_questions : QuestionnaireVerifierTestsContext
    {
        Establish context = () =>
        {
            ...
            {
                new NumericQuestion() {PublicKey = rosterSizeLevel1Id, IsInteger = true, MaxValue = 5},
                new Group()
                {
                    ...
                    {
                        new NumericQuestion() {PublicKey = rosterSizeLevel2Id, IsInteger = true, MaxValue = 5},
                        new Group()
                        {
                            PublicKey = Guid.NewGuid(),
                            IsRoster = true,
                            ...
                            Children = new List<IComposite>()
                            {
                                new NumericQuestion() {PublicKey = rosterSizeLevel3Id, IsInteger = true, MaxValue = 5},
                                new Group()
                                {
                                    PublicKey = Guid.NewGuid(),
                                    IsRoster = true,
                                    ...
                                    Children = new List<IComposite>()
                                    {
                                        new NumericQuestion() {PublicKey = rosterSizeLevel4Id, IsInteger = true, MaxValue = 5},
                                        new Group()
                                        {
                                            PublicKey = Guid.NewGuid(),
                                            IsRoster = true,
