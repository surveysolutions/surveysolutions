namespace WB.Core.GenericSubdomains.Portable.Implementation.Crypto
{
    internal class SHA0 : BlockHash, ICryptoNotBuildIn
    {
        ...
        {
            ulong bits = this.m_processed_bytes * 8;
            int padindex = (this.m_buffer.Pos < 56) ? (56 - this.m_buffer.Pos) : (120 - this.m_buffer.Pos);
        ...
        protected override void TransformBlock(byte[] a_data, int a_index)
        {
            ...
            Converters.ConvertBytesToUIntsSwapOrder(a_data, a_index, this.BlockSize, data, 0);

            this.Expand(data);

            uint A = this.m_state[0];
            uint B = this.m_state[1];
            uint C = this.m_state[2];
            uint D = this.m_state[3];
            uint E = this.m_state[4];
            ...
            this.m_state[0] += data[r++] + SHA0.C4 + ((T7 << 5) | (T7 >> (32 - 5))) + (T6 ^ X5 ^ X4) + X3;
            this.m_state[1] += T7;
            this.m_state[2] += ((T6 << 30) | (T6 >> (32 - 30)));
            this.m_state[3] += X5;
            this.m_state[4] += X4;
        }
        ...
        {
            return Converters.ConvertUIntsToBytesSwapOrder(this.m_state);
        }
    }
}
