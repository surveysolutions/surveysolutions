...
namespace WB.Core.GenericSubdomains.Rest
{
    public interface IRestServiceWrapper
    {
        ...
        void ExecuteRestRequestAsync(string url, CancellationToken ct, byte[] file, string fileName, string login, string password, string method, params KeyValuePair<string, string>[] additionalParams);
    }
}