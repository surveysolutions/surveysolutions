...
namespace WB.Core.SharedKernels.DataCollection.Implementation.Aggregates
{
    internal class Interview : AggregateRootMappedByConvention, ISnapshotable<InterviewState>
    {
        ...
        private  IEnumerable<RosterCalculationData> CalculateDynamicRostersData(InterviewStateStructures state, IQuestionnaire questionnaire, decimal[] outerRosterVector, Guid rosterId)
        {
            ...
            {
                var rosterInstanceIds = GetRosterInstancesById(state, questionnaire, nestedRosterId, outerRosterVector);
            ...
            decimal[] outerRosterVector)
        {
            ...
            var answerOnRosterSizeQuestion =
                GetEnabledQuestionAnswerSupportedInExpressions(state, rosterSizeQuestionIdentity);
            ...
            out List<RosterIdentity> rosterInstancesToAdd, out List<RosterIdentity> rosterInstancesToRemove, out List<RosterCalculationData> rosterInstantiatesFromNestedLevels)
        {
            ...
            foreach (var rosterId in rosterIds)
            {
                ...
                var outerVectorsForExtend =
                    GetOuterVectorForParentRoster(state,rosterGroupsStartingFromTop.Skip(rosterGroupsStartingFromTop.Length - length).ToArray(),
                ...
                foreach (var outerVectorForExtend in outerVectorsForExtend)
                {
                    ...
                    var listOfRosterInstanceIdsForRemove =
                        GetRosterInstanceIdsBeingRemoved(GetRosterInstanceIds(state,rosterId, outerVectorForExtend), rosterInstanceIds).Select(rosterInstanceId =>
                    ...
                    foreach (var rosterInstanceIdBeingChanged in rosterInstancesIdBeingChanged)
                    {
                        ...
                        rosterInstantiatesFromNestedLevels.AddRange(CalculateDynamicRostersData(state, questionnaire, outerRosterVector, rosterId).ToList());
                    }
                }
            }
        }
