select cast(CreatedDate as date), RoleId, SUM(Count) as Count from (
	select 
		ass.CreatedAtUtc + (@minutesoffset ||' minutes')::interval as CreatedDate, 
		ur."RoleId" as RoleId,
		ass.Quantity-(select count(*) from readside.InterviewSummaries isum where ass.Id=isum.assignmentid) as Count
