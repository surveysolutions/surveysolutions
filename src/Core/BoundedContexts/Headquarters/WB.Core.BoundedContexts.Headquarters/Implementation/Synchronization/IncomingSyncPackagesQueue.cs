...
namespace WB.Core.BoundedContexts.Headquarters.Implementation.Synchronization
{
    ...
    internal class IncomingSyncPackagesQueue : InterviewPackagesService
    {
        ...
        public override void StorePackage(string item)
        {
            ...
            if (this.archiver.IsZipStream(new MemoryStream(Encoding.UTF8.GetBytes((string) (syncItem.MetaInfo ?? "")))))
            {
                syncItem.MetaInfo = this.archiver.DecompressString(syncItem.MetaInfo);
            }
            else
            {
                try
                {
                    var decompressedMeta = this.archiver.DecompressString(syncItem.MetaInfo);
            ...
            if (this.archiver.IsZipStream(new MemoryStream(Encoding.UTF8.GetBytes((string) (syncItem.Content ?? "")))))
            {
            ...
            else
            {
                try
                {
                    var decompressedContent = this.archiver.DecompressString(syncItem.Content);
        ...
        private ContextualPolicy SetupRetryPolicyForPackage(string pathToPackage)
        {
            return Policy
                ...
                .WaitAndRetry(
                    this.syncSettings.RetryCount,
                    retryAttempt => TimeSpan.FromSeconds(this.syncSettings.RetryIntervalInSeconds),
        ...
        public override void ProcessPackage(string pathToPackage)
        {
            ...
            try
            {
                ...
                var policy = this.SetupRetryPolicyForPackage(pathToPackage);
                
                string fileContent = policy.Execute(() => this.fileSystemAccessor.ReadAllText(pathToPackage));
