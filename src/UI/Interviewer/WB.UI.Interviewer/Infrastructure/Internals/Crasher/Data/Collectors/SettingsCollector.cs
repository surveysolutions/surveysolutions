...
namespace WB.UI.Interviewer.Infrastructure.Internals.Crasher.Data.Collectors
{
    /// <summary>
    ...
    static class SettingsCollector
    {
        ...
        public static string CollectSystemSettings(Context ctx)
        {
            ...
            var fields = typeof(Android.Provider.Settings.System).GetFields(BindingFlags.Static | BindingFlags.Public);
            foreach (var field in fields)
            {
                if (field.FieldType == typeof(string))
                {
                    try
                    {
                        var value = Android.Provider.Settings.System.GetString(ctx.ContentResolver, (string)field.GetValue(null));
                        if (value != null)
                        {
        ...
        {
            var result = new StringBuilder();
            var fields = typeof(Android.Provider.Settings.Secure).GetFields(BindingFlags.Static | BindingFlags.Public);
            foreach (var field in fields)
            {
                if (field.FieldType != typeof(string)) continue;
                try
                {
                    var value = Android.Provider.Settings.Secure.GetString(ctx.ContentResolver, (string)field.GetValue(null));
                    if (value != null)
                    {
                        result.AppendFormat("{0}={1}", field.Name, value).AppendLine();
                    }
