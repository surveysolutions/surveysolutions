...
const locales = {
    hq: ['Assignments', 'Common', 'Dashboard', 'DataExport', 'DataTables',
        ...
        , 'Roles', 'Workspaces', 'ReviewInterview'],
...
function copyTargets(targets) {
    for (const target of targets) {
        ...
        const sources = fg.sync(normalizePath(target.src))
            ...
            rolldownOptions: {
                ...
                output: {
                    ...
                    manualChunks(id) {
                        ...
                        // NOTE: Do NOT assign ag-grid-community/ag-grid-vue3 to a manual chunk name here.
                        ...
                        // runtime is needed by every component, that forces ag-grid to load
                        // eagerly on every page. Without a manual chunk assignment, Rolldown
                        // keeps ag-grid in the async chunks generated by defineAsyncComponent
                        // in questions/index.js (TableRoster, MatrixRoster), so it is only
                        // fetched when a roster question is first rendered.
                        if (id.includes('/axios/')) return 'vendor-axios-core'
