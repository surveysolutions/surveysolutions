<template>
    <div class="btn-group btn-input clearfix">
        <button type="button" class="btn dropdown-toggle" :disabled="disabled" data-bs-toggle="dropdown">
            <span data-bind="label" v-if="value === null" class="gray-text">{{ watermarkText }}</span>
            <span data-bind="label" v-else>{{ value.title }}</span>
        </button>
        <ul ref="dropdownMenu" class="dropdown-menu" role="menu">
            <li>
                <input type="text" ref="searchBox" :id="searchBoxId" :placeholder="$t('WebInterviewUI.Search')"
