                ...
                <div class="field" :class="{answered: $me.isAnswered}" v-if="$me.isAnswered">
                    <div class="block-with-data">
                        <ul class="list-unstyled">
                            <li
                                v-if="isPolygon"
                            >{{this.$t('Pages.AreaQestion_Area')}}: {{$me.answer.area}} {{this.$t('Pages.AreaQestion_AreaUnitMeter')}}</li>
                            <li
                                v-if="isPolygon || isPolyline"
                            >{{this.$t('Pages.AreaQestion_Length')}}: {{$me.answer.length}} {{this.$t('Pages.AreaQestion_AreaMeter')}}</li>
                            <li
                                v-if="isMultiPoints"
                            >{{this.$t('Pages.AreaQestion_Points')}}: {{$me.answer.selectedPoints.length}}</li>
                            <li v-if="!coordinatesShown">
                                <button
                                    class="btn btn-link"
                                    type="button"
                                    @click="showCoordinates"
                                >{{this.$t('Details.Area_ShowCoordinates')}}</button>
                            </li>
                            <li
                                v-else
                            ...
                            >
                                <a
                                    ...
                                    target="_blank"
                                >{{selectedPoint.latitude}}, {{selectedPoint.longitude}}</a>
                            </li>
                        </ul>
                    </div>
