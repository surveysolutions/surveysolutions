    ...
    methods: {
        answerDoubleQuestion(evnt) {
            ...
            if (answerString.replace(/[^0-9]/g, '').length > 15) {
                this.markAnswerAsNotValidWithMessage(this.$t('WebInterviewUI.DecimalTooBig'), answerString)
                return
            }
        ...
        saveAnswer(answer, isSpecialValue) {
            this.sendAnswer(() => {
                ...
                if (answer > 999999999999999 || answer < -999999999999999) {
                    this.markAnswerAsNotValidWithMessage(this.$t('WebInterviewUI.DecimalCannotParse'), answer)
                    return
                }
