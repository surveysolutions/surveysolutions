            ...
            <div class="options-group">
                <div class="field" :class="{ answered: $me.isAnswered }" v-if="$me.isAnswered">
                    <div class="block-with-data">
                        <table class="list-unstyled" role="presentation">
                            <colgroup>
                            ...
                            <tbody>
                                <tr v-if="isPolygon">
                                    ...
                                    <td>{{ $me.answer.area.toLocaleString() }}
                                        {{ this.$t('Pages.AreaQestion_AreaUnitMeter') }}</td>
                                </tr>
                                <tr v-if="isPolygon || isPolyline">
                                    <td colspan="2">
                                        {{ this.$t('Pages.AreaQestion_Length') }}:
                                    </td>
                                    <td>{{ $me.answer.length.toLocaleString() }}
                                        {{ this.$t('Pages.AreaQestion_AreaMeter') }}</td>
                                </tr>
                                <tr>
                                    <td colspan="2">
                                ...
                                <template v-else>
                                    ...
                                    <tr>
                                        <td></td>
                                        <td colspan="2">
                                            <span>{{ this.$t('Pages.GPS_latitude') }} &nbsp;
                                                {{ this.$t('Pages.GPS_longitude') }}</span>
                                        </td>
                                    </tr>
                                    <tr v-for="(selectedPoint, i) in $me.answer.selectedPoints" :key="i">
                                        <td>{{ i + 1 }}</td>
                                        <td colspan="2">
                                            <a v-bind:href="`${$config.googleMapsBaseUrl}/maps?q=${selectedPoint.latitude},${selectedPoint.longitude}`"
                                                :title="$t('WebInterviewUI.ShowOnMap')"
                                                target="_blank">{{ selectedPoint.latitude.toFixed(6) }},
                                                {{ selectedPoint.longitude.toFixed(6) }}</a>
                                        </td>
                                    </tr>
                                </template>
                            </tbody>
                ...
                <div class="action-btn-holder">
                    <button type="button" disabled class="btn btn-default btn-lg btn-action-questionnaire">{{
                        $t('WebInterviewUI.AreaRecord') }}</button>
                </div>
                <wb-lock />
            </div>
        </div>
