        ...
        <div class="mb-30">
            <div class="col-md-12">
                <form class="form-container" data-vv-scope="settings">
                    ...
                    <div class="form-inline">
                        <div class="form-group" :class="{ 'has-error': errors.has('settings.senderAddress') }">
                            ...
                            <div class="field" :class="{ 'answered': senderAddress }">
                                ...
                                <span class="gray-text help-block">{{ $t('Settings.EmailProvider_SenderHelp') }}</span>
                        ...
                        <div class="form-group" :class="{ 'has-error': errors.has('settings.replyAddress') }">
                            ...
                            <div class="field" :class="{ 'answered': replyAddress }">
                                <input data-vv-as="reply email address" v-validate="'email'" name="replyAddress"
                                    id="replyAddress" v-model="replyAddress" type="text" class="form-control with-clear-btn"
                                    maxlength="200" />
                                <button type="button" @click="replyAddress = null" class="btn btn-link btn-clear">
                                    <span></span>
                                </button>
                                <span class="gray-text help-block">{{ $t('Settings.EmailProvider_ReplyAddressHelp')
                                }}</span>
                    ...
                    <div class="form-group mb-30" :class="{ 'has-error': errors.has('settings.address') }">
                        ...
                        <div class="field" :class="{ 'answered': address }">
                            <input data-vv-as="address" v-validate="'required_if:provider,amazon,sendgrid'" name="address"
                                id="address" v-model="address" type="text" class="form-control with-clear-btn"
                                maxlength="200" />
                    ...
                    <div class="radio-accordion mb-30">
                        <div class="radio mb-1">
                            <input v-validate="'required'" name="provider" class="wb-radio" type="radio" v-model="provider"
                                    ...
                                    <div class="form-group" :class="{ 'has-error': errors.has('settings.awsAccessKeyId') }">
                                        ...
                                        <div class="field" :class="{ 'answered': awsAccessKeyId }">
                                            <input data-vv-as="AWS access key id" v-validate="'required'"
                                                ...
                                                id="awsAccessKeyId" type="text" v-model="awsAccessKeyId" maxlength="200" />
                                            <button @click="awsAccessKeyId = null" type="button"
                                                class="btn btn-link btn-clear">
                                                <span></span>
                                            </button>
                                            <span class="gray-text help-block">{{
                                                $t('Settings.EmailProvider_AwsAccessKeyIdHelp') }}</span>
                                    ...
                                    <div class="form-group"
                                        ...
                                        <div class="field" :class="{ 'answered': awsSecretAccessKey }">
                                            <input v-validate="'required'" data-vv-as="AWS secret access key"
                                                ...
                                                v-model="awsSecretAccessKey" class="form-control with-clear-btn" type="text"
                                                maxlength="200" />
                                            ...
                                            <span class="gray-text help-block">{{
                                                $t('Settings.EmailProvider_AwsSecretAccessKeyHelp') }}</span>
                                            <span class="help-block">{{ errors.first('settings.awsSecretAccessKey')
                                            }}</span>
                                        </div>
                                    </div>

                                    <div class="form-group" :class="{ 'has-error': errors.has('settings.awsRegion') }">
                                        <label class="h5">{{ $t('Settings.EmailProvider_AwsRegion') }}</label>
                                        <div class="field" :class="{ 'answered': awsRegion }">
                                            <select v-validate="'required'" data-vv-as="AWS region" name="awsRegion"
                                                ...
                                                <option :key="awsRegion.key" :value="awsRegion.key"
                                                    v-for="awsRegion in $config.model.awsRegions"
                                                    v-html="awsRegion.value" />
                                            </select>
                                            <span class="gray-text help-block">{{
                                                $t('Settings.EmailProvider_AwsRegionHelp') }}</span>
                                            <span class="help-block">{{ errors.first('settings.awsRegion') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="wrapper">
                                    ...
                                    <div class="form-group" :class="{ 'has-error': errors.has('settings.sendGridApiKey') }">
                                        ...
                                        <div class="field" :class="{ 'answered': sendGridApiKey }">
                                            ...
                                            <span class="gray-text help-block">{{
                                                $t('Settings.EmailProvider_SendGridApiKeyHelp') }}</span>
                    ...
                    <p class="text-info" v-if="isFormDirty">Save current changes and send yourself a test email to verify
                        that the bulk email service is functional</p>
                    <div class="form-group">
                ...
                <form v-if="!isFormDirty && (sendGridIsSetUp || awsIsSetUp)" data-vv-scope="testEmail"
                    ...
                    <p class="text-success" v-if="sendEmailResult">{{ $t('Settings.EmailProvider_SendTestEmailResult') }}
                    </p>
    ...
    methods: {
        ...
        async save() {
            ...
            if (validationResult) {
                ...
                this.$http
                    .post(this.$config.model.api.updateSettings,
                        settings,
                        {
                            headers: {
                                'Content-Type': 'multipart/form-data',
