    ...
    <div class="row">
      <div class="col-sm-7 col-xs-12 action-block">
        ...
        <div class="error-block" v-for="(error, index) in verificationErrors" :key="index">
          ...
          <div v-for="(reference, index) in error.references" :key="index">
            ...
            <p v-if="!isEmptyText(reference.content)">
              <span v-if="reference.type == 0">{{$t('BatchUpload.Column')}}: {{reference.content}}</span>
              <span v-if="reference.type == 1">{{$t('BatchUpload.Row')}}: {{reference.content}}</span>
              <span v-if="reference.type == 2">{{$t('BatchUpload.Cell')}}: {{reference.content}}</span>
              <span v-if="reference.type == 3">{{$t('BatchUpload.File')}}: {{reference.content}}</span>
        ...
        <div class="action-buttons">
          <a class="back-link" v-bind:href="assignmentsUploadUrl">{{$t('BatchUpload.BackToImport')}}</a>
        </div>
      </div>
    </div>
  </HqLayout>
