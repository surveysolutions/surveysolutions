<template>
    <HqLayout :hasFilter="false">
        ...
        <div class="row">
            <div class="col-sm-7">
                <h3>{{$t('BatchUpload.ImportFrom', { fileName: fileName, title: $t('Pages.QuestionnaireNameFormat', { name: questionnaire.title, version: questionnaire.version }) })}}</h3>
            </div>
        </div>
        <div class="row">
            <div class="col-sm-7 col-xs-12 action-block">
                ...
                <h3
                    class="error-text"
                    v-if="verificationErrors.length == 1"
                >{{$t('BatchUpload.SingleErrorFound')}}</h3>
                <h3
                    class="error-text"
                    v-else
                >{{$t('BatchUpload.MultipleErrorsWereFound', {count: verificationErrors.length})}}</h3>
                ...
                <div class="error-block" v-for="error in verificationErrors">
                    ...
                    <div v-for="reference in error.references">
                        <p>{{$t('BatchUpload.FileName')}}: {{reference.dataFile}}</p>
                        <p v-if="!isEmptyText(reference.content)">
                            <span
                                v-if="reference.type == 0"
                            >{{$t('BatchUpload.Column')}}: {{reference.content}}</span>
                            <span
                                v-if="reference.type == 1"
                            >{{$t('BatchUpload.Row')}}: {{reference.content}}</span>
                            <span
                                v-if="reference.type == 2"
                            >{{$t('BatchUpload.Cell')}}: {{reference.content}}</span>
                            <span
                                v-if="reference.type == 3"
                            >{{$t('BatchUpload.File')}}: {{reference.content}}</span>

                            <span v-else>{{reference.type}}: {{reference.content}}</span>
                            (
                            <span
                                v-if="!isEmptyText(reference.column)"
                            >{{$t('BatchUpload.Column')}}: {{reference.column}},</span>
                            <span
                                v-if="reference.row!=null"
                            >{{$t('BatchUpload.Row')}}: {{reference.row}}</span>)
                        </p>
                    </div>
                </div>
