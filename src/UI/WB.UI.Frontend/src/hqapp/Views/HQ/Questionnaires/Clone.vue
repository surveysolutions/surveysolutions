        ...
        <div class="row">
            <form method="POST" class="col-sm-8" @submit.prevent="tryClone" novalidate ref="frmClone">
                <div class="form-group" :class="{'has-error': errors.has('NewTitle')}">
                    <label for="NewTitle" class="control-label">{{$t('FieldsAndValidations.CloneQuestionnaireModel_NewTitle_Label')}}</label>
                    <input type="text" id="NewTitle" name="NewTitle" v-validate="'required'" class="form-control" autocomplete="off"/>
                ...
                <div class="form-group">
                    <label for="Comment" class="control-label">{{$t('Assignments.DetailsComments')}}</label>
                    <textarea name="Comment" id="Comment" class="form-control" :placeholder="$t('Assignments.EnterComments')" rows="6" maxlength="500"></textarea>
                </div>
                <input
                ...
                <input
                    name="OriginalTitle"
                    type="hidden"
                    :value="$config.model.originalTitle"
                />
                <input
                    name="IsCensus"
                    type="hidden"
                ...
                <div class="action-buttons">
                    <button type="submit" class="btn btn-success">{{$t('QuestionnaireClonning.Clone')}}</button>
                    <a href="../../SurveySetup" class="back-link">{{$t('Common.Cancel')}}</a>
                </div>
            </form>
        </div>
        </HqLayout>
