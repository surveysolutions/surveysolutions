...
<div>
    ...
    <form :action="$config.model.submitUrl" method="post">
        ...
        <div v-if="model.useCaptcha && !model.serverUnderLoad" class="form-group">
            <vue-recaptcha v-if="model.useCaptcha" sitekey="model.recaptchaSiteKey" :loadRecaptchaScript="true">
            </vue-recaptcha>
        </div>
        ...
        <div v-else class="row-element mb-20">
            <button class="btn btn-success btn-lg" type="submit">{{ buttonTitle }}</button>
        </div>

        <input v-if="model.hasPendingInterviewId" name="resume" class="btn btn-success btn-lg" type="submit" value="$t('WebInterview.ResumeInterview')" />
    </form>
</div>
