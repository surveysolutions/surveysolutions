<template>
    <div class="rule-slot vqb-rule card">
        <div class="form-inline">
            ...
            <select v-if="typeof rule.operands !== 'undefined'"
                v-model="query.operand"
                class="form-control mr-2 mb-5">
                <option v-for="operand in rule.operands"
                    :key="operand">
                    {{ operand }}
                </option>
            </select>
            ...
            <select v-if="typeof rule.operators !== 'undefined' && rule.operators.length > 1"
                v-model="query.operator"
                class="form-control mr-2 mb-5">
                <option v-for="operator in rule.operators"
                    :key="operator"
                    :value="operator">
                    {{ operator }}
                </option>
            ...
            <!-- Basic text input -->
            <input v-if="rule.type === 'text' && !unaryOperatorSelected"
                v-model="query.value"
            ...
            <!-- Basic number input -->
            <input v-if="rule.type === 'numeric' && !unaryOperatorSelected"
                v-model="query.value"
                class="form-control mb-5"
                type="number">

            <!-- Datepicker -->
            <input v-if="rule.type === 'date' && !unaryOperatorSelected"
                v-model="query.value"
                class="form-control mb-5"
                type="date">
            ...
            <template v-if="rule.type === 'checkbox' && !unaryOperatorSelected">
                <div v-for="choice in rule.choices"
                    :key="choice.value"
            ...
            <select v-if="rule.type === 'select' && !hasOptionGroups && !unaryOperatorSelected"
                v-model="query.value"
                class="form-control mb-5"
                :multiple="rule.type === 'multi-select'">
                <option v-for="option in rule.choices"
                    :key="option.value"
                    :value="option.value">
                    {{ option.label }}
                </option>
            ...
            <select v-if="rule.type === 'select' && hasOptionGroups && !unaryOperatorSelected"
                ...
                class="form-control mb-5"
                :multiple="rule.type === 'multi-select'">
                <optgroup v-for="(option, option_key) in rule.selectOptions"
                    :key="option_key"
                    :label="option_key">
                    <option v-for="sub_option in option"
