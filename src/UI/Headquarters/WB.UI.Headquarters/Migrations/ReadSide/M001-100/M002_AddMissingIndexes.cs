...
namespace WB.UI.Headquarters.Migrations.ReadSide
{
    ...
    public class M002_AddMissingIndexes : Migration
    {
        public override void Up()
        {
            ...
            {
                Execute.Sql("CREATE INDEX answerstofeaturedquestions_answervalue ON answerstofeaturedquestions (answervalue text_pattern_ops)");
            }

            if (!Schema.Table("interviewdataexportrecords").Index("interviewdataexportrecords_id_text_pattern_ops_idx").Exists())
            {
                Execute.Sql("CREATE INDEX interviewdataexportrecords_id_text_pattern_ops_idx ON interviewdataexportrecords(id text_pattern_ops)");
            }
        }
