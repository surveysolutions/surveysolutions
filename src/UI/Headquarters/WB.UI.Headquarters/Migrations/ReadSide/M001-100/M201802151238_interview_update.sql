...
CREATE OR REPLACE FUNCTION readside.interview_update(id uuid,
    ...
    errorvalidations readside.interviewvalidation[],
    warningvalidations readside.interviewvalidation[],
...
$BODY$
  ...
  INSERT INTO readside.interviews (interviewid, entityid, rostervector, isreadonly)
                ...
                isreadonly = true;
  END LOOP;
  FOREACH validation IN ARRAY errorvalidations
  LOOP
  ...
  END LOOP;
  FOREACH validation IN ARRAY warningvalidations
  LOOP
  INSERT INTO readside.interviews (interviewid, entityid, rostervector, warnings)
                VALUES(id, validation.id, validation.rostervector, validation.validations)
                ON CONFLICT ON CONSTRAINT pk_interviews
                DO UPDATE SET
                warnings = validation.validations;
  END LOOP;
