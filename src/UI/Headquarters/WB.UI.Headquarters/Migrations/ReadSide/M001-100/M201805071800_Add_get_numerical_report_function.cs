...
namespace WB.UI.Headquarters.Migrations.ReadSide
{
    ...
    public class M201805071800_Add_get_numerical_report_function : Migration
    {
        public override void Up()
        {
            ...
            Execute.Sql(@"CREATE OR REPLACE FUNCTION readside.get_numerical_report(_entityid uuid, _questionnaireidentity text, _teamleadid uuid, detailed boolean, minanswer integer, maxanswer integer)
                 RETURNS TABLE(teamleadname text, responsiblename text, count bigint, avg numeric, median numeric, min integer, max integer, sum bigint, percentile_05 double precision, percentile_50 double precision, percentile_95 double precision)
                 LANGUAGE plpgsql
                AS $function$
                                BEGIN
                                     RETURN QUERY
