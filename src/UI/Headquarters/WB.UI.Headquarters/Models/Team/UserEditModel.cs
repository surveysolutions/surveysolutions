...
namespace WB.UI.Headquarters.Models.Team
{
    public class UserEditModel
    {
        ...
        [RegularExpression("(?=^.{6,255}$)((?=.*[a-z])(?=.*[0-9])(?=.*[A-Z]))^.*", 
            ErrorMessage = "Password must contain at least one number, one upper case character and one lower case character. Length must be between 6 and 255 characters")]
