...
namespace WB.UI.Headquarters.API.DataCollection.Supervisor.v1
{
    public class InterviewersApiController : ApiController
    {
        ...
        {
            var users = this.userViewFactory.GetInterviewers(this.authorizedUser.Id).ToList();
            users.ForEach(x =>x.Token = this.signInManager.GenerateApiAuthTokenAsync(x.Id).Result);
