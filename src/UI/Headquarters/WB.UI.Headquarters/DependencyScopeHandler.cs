...
namespace WB.UI.Headquarters
{
    ...
    class DependencyScopeHandler : DelegatingHandler
    {
        ...
        public DependencyScopeHandler(ILifetimeScope autofacLifetimeScope)
        {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (this.autofacLifetimeScope == null) return base.SendAsync(request, cancellationToken);
            
            var dependencyScope = new AutofacWebApiDependencyScope(this.autofacLifetimeScope);
    ...
    public class HqBufferPolicySelector : IHostBufferPolicySelector
    {
        /// <inheritdoc />
        ...
        public bool UseBufferedOutputStream(HttpResponseMessage response)
        {
            ...
            if (transferEncodingChunked.HasValue && transferEncodingChunked.Value) return false;

            // Content length is null or -1 (meaning not known).  
            // Buffer any HttpContent except StreamContent and PushStreamContent
            if (content is StreamContent || content is PushStreamContent) return false;
