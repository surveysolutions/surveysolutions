...
const favicons = () => src(config.vendor.favicons).pipe(dest(config.dist));

const assets = () =>
    merge(
        src(config.assets.images).pipe(dest(join(config.dist, 'images'))),
        src(config.assets.fonts).pipe(dest(join(config.dist, 'fonts'))),
        src(config.assets.qbank).pipe(dest(join(config.dist, 'qbank')))
    );
...
module.exports = {
    ...
    bowerCss,
    bowerImages,
    default: parallel(favicons, bowerCss, bowerImages, assets),
};
