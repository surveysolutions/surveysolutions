    ...
    element.fancytree({
        ...
        contextMenu: {
            ...
            menu: function (node) {
                ...
                if (node.key == 'root')
                    return {
                        createSubFolder: {
                            name: localization.CreateSubFolder,
                            icon: 'add',
                        },
                    };
            ...
            actions: function (node, action, options) {
                if (action === 'createSubFolder') {
                    ...
                    self.postRequest(
                        addNodeUrl,
                        { parentId: node.key, title: defaultFolderName },
                        function (data) {
                    ...
                    var message = localization.DeleteConfirmation.replace(
                        '{0}',
                        "'" + node.title + "'"
                    );
        ...
        edit: {
            ...
            beforeEdit: function (event, data) {
                ...
                if (data.node.key === 'root') return false;
            },
            ...
            save: function (event, data) {
                ...
                self.postRequest(
                    renameNodeUrl,
                    { id: data.node.key, newTitle: newTitle },
                    function () {
...
window.FoldersModel = FoldersModel;
