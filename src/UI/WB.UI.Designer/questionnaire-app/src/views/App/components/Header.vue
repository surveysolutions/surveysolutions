    ...
    <div class="header-line">
      <div class="header-menu">
        <div class="buttons">
          ...
          <button class="btn" type="button" v-disabled="questionnaire.isReadOnlyForUser && !questionnaire.hasViewerAdminRights && !questionnaire.isSharedWithUser" 
            @click="showShareInfo()">
...
">
              <span data-toggle="modal" v-if="verificationStatus.warnings.length + verificationStatus.errors.length > 0" 
                class="error-message v-hide" 
                v-class="{ 'no-errors': verificationStatus.errors.length == 0 }">
                <a href="javascript:void(0);" @click="showVerificationErrors()">
                  {{ $t('QuestionnaireEditor.ErrorsCounter', { count: verificationStatus.errors.length}) }}
                </a> 
              </span>
              ...
              <span class="text-success" v-if="verificationStatus.warnings.length + verificationStatus.errors.length === 0">
            ...
            <button id="webtest-btn" type="button" class="btn" v-if="questionnaire.webTestAvailable && questionnaire.questionnaireRevision === null" 
              @click="webTest()">
...
export default {
  ...
  methods: {
    async fetch() {
      ...
      await this.questionnaireStore.fetchQuestionnaireInfo(this.questionnaireId)
    },
    webTest() {
