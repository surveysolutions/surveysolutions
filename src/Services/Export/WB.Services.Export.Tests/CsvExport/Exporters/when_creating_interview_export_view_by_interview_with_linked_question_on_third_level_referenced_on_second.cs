    ...
    internal class when_creating_interview_export_view_by_interview_with_linked_question_on_third_level_referenced_on_second : ExportViewFactoryTestsContext
    {
        [NUnit.Framework.OneTimeSetUp] public void context () {
            ...
            if (!rosterLevel.QuestionsSearchCache.ContainsKey(linkedQuestionId))
                rosterLevel.QuestionsSearchCache.Add(linkedQuestionId, Create.InterviewEntity(identity: Create.Identity(linkedQuestionId), asIntArray: new int[]{0}));
        ...
        [NUnit.Framework.Test] public void should_linked_question_have_one_answer () =>
           GetLevel(result, new[] { rosterId, nestedRosterId }).Records[0].GetPlainAnswers().First().Length.Should().Be(1);

        [NUnit.Framework.Test] public void should_linked_question_have_first_answer_be_equal_to_0 () =>
           GetLevel(result, new[] { rosterId, nestedRosterId }).Records[0].GetPlainAnswers().First().First().Should().Be("0");
