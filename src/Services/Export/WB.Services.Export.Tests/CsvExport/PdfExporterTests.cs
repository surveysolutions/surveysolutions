...
namespace WB.Services.Export.Tests.CsvExport
{
    ...
    public class PdfExporterTests
    {
        ...
        public async Task should_export_pdf_file()
        {
            ...
            fileSystem.Setup(x => x.OpenOrCreateFile($"testPath{Path.DirectorySeparatorChar}Pdf{Path.DirectorySeparatorChar}{questionnaire.VariableName}.pdf", false))
                .Returns(mockMainStream);
            fileSystem.Setup(x => x.OpenOrCreateFile($"testPath{Path.DirectorySeparatorChar}Pdf{Path.DirectorySeparatorChar}language name {questionnaire.VariableName}.pdf", false))
