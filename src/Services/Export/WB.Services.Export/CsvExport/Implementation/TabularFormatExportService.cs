...
namespace WB.Services.Export.CsvExport.Implementation
{
    public class TabularFormatExportService : ITabularFormatExportService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            await this.interviewsExporter.ExportAsync(tenant, questionnaireExportStructure, questionnaire, interviewsToExport, tempPath, exportInterviewsProgress, cancellationToken, settings.GeographyExportFormat);
        ...
        public Task GenerateShortDescriptionFileAsync(TenantInfo tenant, QuestionnaireId questionnaireId, string basePath, string dataFilesExtension, DataExportFormat dataExportFormat, bool? paradataReduced, CancellationToken cancellationToken) => 
            WriteDescriptionFileAsync(tenant, questionnaireId, basePath, dataFilesExtension, true, dataExportFormat, paradataReduced, cancellationToken);
        ...
        public Task GenerateDescriptionFileAsync(TenantInfo tenant, QuestionnaireId questionnaireId, string basePath, string dataFilesExtension, CancellationToken cancellationToken) => 
            WriteDescriptionFileAsync(tenant, questionnaireId, basePath, dataFilesExtension, false, null, false, cancellationToken);

        private async Task WriteDescriptionFileAsync(TenantInfo tenant, QuestionnaireId questionnaireId, string basePath, 
            string dataFilesExtension, bool isCaptionOnly, 
            DataExportFormat? dataExportFormat, bool? paradataReduced, CancellationToken cancellationToken)
        {
            ...
            if (dataExportFormat == DataExportFormat.Paradata)
            {
                if (paradataReduced == true)
                    descriptionBuilder.AppendLine($"Reduced paradata set does not include events with the following codes: QuestionEnabled (13), QuestionDisabled (14), GroupEnabled (15), GroupDisabled (16), QuestionDeclaredValid (17), QuestionDeclaredInvalid (18), VariableEnabled (23), VariableDisabled (24).");
                else
