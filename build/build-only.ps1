...
try {
    ...
    if ($buildSuccessful) { 
        ...
        BuildStaticContent "Hq UI" "src\UI\Headquarters\WB.UI.Headquarters\InterviewerApp" | % { if (-not $_) {
                Write-Host "##teamcity[message status='ERROR' text='Unexpected error occurred in BuildStaticContent']"
                Write-Host "##teamcity[buildProblem description='Failed to build static content for HQ']"
                Exit 
        } else {
            Move-Item "..\WB.UI.Headquarters\InterviewerApp\dist\stats.html" "$artifactsFolder\Hq.UI.stats.html" -ErrorAction SilentlyContinue
        }}
