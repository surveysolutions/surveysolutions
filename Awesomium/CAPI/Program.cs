...
namespace Browsing.CAPI
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (Mutex mutex = new Mutex(true, "CAPI", out createdNew))
            {
                if (!createdNew)
                {
                    MessageBox.Show("One copy of the CAPI application is already running", "Launch error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
